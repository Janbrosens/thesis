...
static sgx_status_t SGX_CDECL sgx_ecall_increase(void* pms)
{
...
static sgx_status_t SGX_CDECL sgx_ecall_lookup(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_lookup_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_lookup_t* ms = SGX_CAST(ms_ecall_lookup_t*, pms);
	ms_ecall_lookup_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ecall_lookup_t), ms, sizeof(ms_ecall_lookup_t))) {
...
SGX_EXTERNC const struct {
	...
	2,
	{
		{(void*)(uintptr_t)sgx_ecall_increase, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_lookup, 0, 0},
	}
};
